...
CONFIG_TEMPLATE = "Pressure_from_velocity/configs/bnn_config.yaml"
OUTPUT_ROOT = "Pressure_from_velocity/outputs/BNN_trials"
SUMMARY_FILE = "bnn_validation_summary.csv"
SBATCH_SCRIPT = "run_fcn.slurm"
trials_names = ['single_bump', 'single_phill', 'bump_inter', 'phill_inter', 'bump_extrap', 'phill_extrap' ]
# Sweep parameters
architectures = {
...
def generate_all_jobs():
    jobs = []
    for lr, out_act, activation, trial, norm, (arch_name, shape) in product(
                             learning_rates,output_activations, hidden_activations,
                             trials_names, norm_types,
                             architectures.items()):

        name    = f"BNN_{trial}_{norm}_{arch_name}_{activation}_lr{lr}"
        ...
        cfg_md = config['model']
        cfg_md['type']       = "BNN"
        cfg_md['branch_layers'] = shape['branch_layers']
        cfg_md['trunk_layers'] = shape['trunk_layers']
        cfg_md['hidden_activation'] = activation
        cfg_md['output_activation'] = out_act
        cfg_md['dropout'] = dropout
        cfg_tr = config['training']
        cfg_tr['epochs']  = epochs
        cfg_tr['norm_labels'] = True
        cfg_tr['norm'] = norm
        cfg_tr['lr'] = lr
