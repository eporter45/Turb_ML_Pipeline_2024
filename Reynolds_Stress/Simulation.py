...
def main(config_path):
    ...
    data = load_data_rst_from_config(config, train_cases, test_cases, data_path)
    ...
    optimizer = None
    print(f"[INFO] Training model type {model_type}", flush=True)
    if model_type == "fcn":
    ...
    try:
        metrics['num_params'] = sum(p.numel() for p in best_model.parameters())
    except:
        print('[ERROR] best_model.parameters() were found in the model.', flush=True)

    from utils.frobenius_error import frobenius_error
    metrics['frobenius_norm'] = frobenius_error(y_pred, y_test)
    ...
    plot_all_case_losses(loss_df, config, loss_plot_dir)
...
# to run call this
#python Reynolds_Stress/Simulation.py \--config Reynolds_Stress/configs/nn_config.yaml
